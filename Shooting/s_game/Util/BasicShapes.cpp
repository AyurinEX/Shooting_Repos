...
void* BasicShapeMesh::positions() const {
	return (void*)&vertices_[0].position;
}
...
void BasicShapeMesh::draw(const VertexAttribute& attribute) const {
	...
	if (attribute.position != -1) {
		...
		glVertexAttribPointer(attribute.position, 3, GL_FLOAT, GL_FALSE, vertexStride(), positions());
	}
	...
	// 頂点データを無効にする
	if (attribute.position != -1) glDisableVertexAttribArray(attribute.position);
...
SphereMesh::SphereMesh(float radius, int slices, int stacks)
	...
	for (int i = 0; i <= slices; ++i) {
		...
		for (int j = 0; j <= stacks; ++j) {
			...
			vertices_[idx].position = GSvector3(vx * radius, vy * radius, 0.0) * ry;
...
TorusMesh::TorusMesh(float innerRadius, float outerRadius, int sides, int rings)
	...
	for (int i = 0; i <= rings; ++i) {
		...
		for (int j = 0; j <= sides; ++j) {
			...
			float vx = gsCos(angle);
			float vy = gsSin(angle);
			vertices_[idx].position = GSvector3(vx * radius + x, vy * radius, 0.0f) * ry;
...
CylinderMesh::CylinderMesh(float radius, float height, int sides)
	...
	for (int i = 0; i <= sides; ++i) {
		...
		for (int j = 0; j < ARRAY_LENGTH(v); ++j) {
			GSvector3 position( v[j].position.x * radius, v[j].position.y * height, v[j].position.z );
			vertices_[idx].position = position * ry;
...
BoxMesh::BoxMesh(float width, float height, float depth)
	...
	for (VertexBuffer::size_type i=0; i < vertices_.size(); ++i) {
		vertices_[i] = v[i];
		vertices_[i].position = GSvector3(v[i].position.x * width, v[i].position.y * height, v[i].position.z * depth); 
	}
	// インデックスの作成
